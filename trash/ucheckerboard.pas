...
type
  TCodeTable = Object
    ...
    letters: widestring;  //all characters, which belongs to different codepages
...
procedure TCodeTable.CreateFromStrings(strs:tStringList);
 ...
 var
   ...
   i:integer;
   buffer:widestring;
   upperbuffer:widestring;
   readerState:integer=SNONE;
   currentCode:widestring;
   currentSymbol:widestring;
 begin
   rawsource:=strs.text;
   ...
   begin
     buffer:= UTF8decode(Trim(strs[i]));
     upperbuffer:=WideUpperCase(buffer);
       ...
       if (readerState = SCODE1) or (readerState = SCODE2) then
         ...
         //extract code and symbol from buffer
          ...
          if readerState = SCODE1 then
            begin
            cp1dec.Add(UTF8encode(currentCode), UTF8encode(currentSymbol));
            cp1enc.Add(UTF8encode(currentSymbol),UTF8encode(currentCode))
            end;
          if readerState = SCODE2 then
            begin
            cp2dec.Add(UTF8encode(currentCode), UTF8encode(currentSymbol));
            cp2enc.Add(UTF8encode(currentSymbol),UTF8encode(currentCode));
            end;
          except
...
function TCodeTable.Encode(itxt:ansistring):ansistring;
  ...
  begin
    //encoding loop
    txt:=WideUpperCase( utf8decode(itxt) );
    for  i:=1 to length(txt) do
    begin
       b:= Copy(txt, i,1 );             //txt[i];
      ...
      if (pos(b, digits)>0) and (not fig) then
        begin
        fig:=True;
        r+=utf8decode(cp1enc['!FIG'])   //????!
        end;
      if (pos(b, digits)=0) and fig then
        begin
        fig:=False;
        r:= r+ utf8decode(cp1enc['!FIG'])
        end;
      //is there a space?
       if b=' ' then
         begin
         r+=utf8decode(cp1enc['!SPACE']);
         Continue
         end;
      ...
      begin
       ...
       if cpage1 then r+=utf8decode(cp1enc[b]);
       if (not cpage1) then
         begin
         if(cp2enc[b]='') then r+=utf8decode(cp1enc[b]) else  r+=utf8decode(cp2enc[b]);  //not so dumb
         end
      end;//end main loop
    Encode:=r
    end;
    ...
    repeat
      ...
      if isDigits then
       ...
       if (blength=2) and (buff[1]=buff[2]) then
        ...
        if buff[1] = utf8decode(cp1enc['!FIG']) then  //if fig. designator encoded with one digit
         begin
         position :=position +1;
         Continue;
         end;
        if buff = utf8decode(cp1enc['!FIG']) then    //if designator encoded with two digits
         begin
         position :=position +2;
         Continue;
         end;
      ...
      if (cp1dec[buff]<>'') and (cp1dec[buff][1]='!') and (length(cp1dec[buff])>1) then
       begin
        ...
        case utf8decode(cp1dec[buff]) of
          '!FIG'  : isDigits:=true;
          '!SPACE': rz+=' ';
          '!CP1' : iscp1:=true;
          '!CP2' : iscp1:=false;
